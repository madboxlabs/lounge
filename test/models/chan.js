...
describe("Chan", function() {
	...
	describe("#setUser(user)", function() {
		...
		it("should update user object", function() {
			...
			expect(chan.getMode("testUSER")).to.equal("@");
		});
	});
	...
	describe("#getMode(nick)", function() {
		...
		it("should return empty string for non existing users", function() {
			const chan = new Chan();

			expect(chan.getMode("test")).to.equal("");
		});
	});
	...
	describe("#getSortedUsers(irc)", function() {
		...
		it("should sort a simple user list", function() {
			...
			chan.setUser(new User({nick: "YaManicKill"}, prefixLookup));
			chan.setUser(new User({nick: "astorije"}, prefixLookup));
			chan.setUser(new User({nick: "xPaw"}, prefixLookup));
			chan.setUser(new User({nick: "Max-P"}, prefixLookup));
		...
		it("should be case-insensitive", function() {
			const chan = new Chan();
			chan.setUser(new User({nick: "aB"}, prefixLookup));
			chan.setUser(new User({nick: "Ad"}, prefixLookup));
			chan.setUser(new User({nick: "AA"}, prefixLookup));
			chan.setUser(new User({nick: "ac"}, prefixLookup));
		...
		it("should parse special characters successfully", function() {
			const chan = new Chan();
			chan.setUser(new User({nick: "[foo"}, prefixLookup));
			chan.setUser(new User({nick: "]foo"}, prefixLookup));
			chan.setUser(new User({nick: "(foo)"}, prefixLookup));
			chan.setUser(new User({nick: "{foo}"}, prefixLookup));
			chan.setUser(new User({nick: "<foo>"}, prefixLookup));
			chan.setUser(new User({nick: "_foo"}, prefixLookup));
			chan.setUser(new User({nick: "@foo"}, prefixLookup));
			chan.setUser(new User({nick: "^foo"}, prefixLookup));
			chan.setUser(new User({nick: "&foo"}, prefixLookup));
			chan.setUser(new User({nick: "!foo"}, prefixLookup));
